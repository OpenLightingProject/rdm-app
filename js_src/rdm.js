angular.module('rdmApp', [])
 ...
 .controller('EUIDController',
  ...
  $scope.convertToUID = function() {
   ...
   var calculated_checksum = checksumService.checksumAsValue(data.slice(0, 12));
 ...
 .controller('RDMPacketBuilder',
  ...
  function commandClassChanged(new_value, old_value, scope) {
    ...
    angular.forEach(pid_names, function(key, index) {
     ...
     var label = key + ' (' + formatService.toHex(RDM.PIDS[key], 4, '0x') + ')';
  ...
  $scope.buildPacket = function() {
    ...
    var dest_uid = parseUIDOrSetError($scope.packet.dest_uid, 'destination');
    if (!dest_uid) {
     return;
    }
    ...
    var param_data = [];
    if ($scope.packet.command_class === RDM.COMMAND_CLASS.DISCOVERY_COMMAND &&
     ...
     } else if ($scope.packet.response_type === RDM.RESPONSE_TYPE.NACK) {
      param_data = [$scope.packet.nack_reason >> 8,
                    $scope.packet.nack_reason & 0xff];
     } else if ($scope.packet.response_type === RDM.RESPONSE_TYPE.ACK_TIMER) {
    ...
    if ($scope.settings.output_format === OUTPUT_FORMAT.C_ARRAY.value) {
     $scope.output = formatService.dataAsArray(data, $scope.settings.wrap);
    } else if ($scope.settings.output_format === OUTPUT_FORMAT.C_STRING.value) {
     $scope.output = formatService.dataAsString(data, $scope.settings.wrap);
    } else if ($scope.settings.output_format === OUTPUT_FORMAT.RAW_HEX.value) {
     $scope.output = formatService.dataAsRawHex(data, $scope.settings.wrap);
    }
   };
 }])
 ...
 .controller('RDMPacketParser',
  ...
  $scope.parsePacket = function() {
   ...
   if (packet_data.length >= 2) {
    var recovered_checksum = (packet_data.shift() << 8) + packet_data.shift();
    $scope.packet.checksum = formatService.toHex(recovered_checksum, 4, '0x');
